...
async function apiFetch(url, options = {}) {
  ...
  try {
    ...
    console.error(`[SpiderWebService] Fetch parse error for ${url}:`, err.message)
    throw err
  }
}
...
class SpiderWebService {
  ...
  async updateUserProfile(userId, fields) {
    ...
    if (fields.artistTags !== undefined && Array.isArray(fields.artistTags)) {
      const safeJson = JSON.stringify(fields.artistTags).replace(/'/g, "''")
      sets.push(`artist_tags = '${safeJson}'`)
    }
  ...
  async getApprovedOverlays({ page = 1, limit = 24, categorySlug, minRating, onlyFree } = {}) {
    ...
    return this.query(
      `SELECT r.*
       ...
       ORDER BY r.avg_rating DESC, r.favorite_count DESC
  ...
  async searchOverlays(term, { page = 1, limit = 24 } = {}) {
    ...
    return this.query(
      `SELECT DISTINCT r.*,
              (CASE WHEN o.name = '${safe}' THEN 3 WHEN o.name LIKE '${safe}%' THEN 2 WHEN o.name LIKE '%${safe}%' THEN 1 ELSE 0 END) AS name_sim
       FROM overlay_ratings r
       JOIN overlays o ON o.id = r.id
       ...
       WHERE o.status = 'APPROVED'
         AND (
           o.name        LIKE '%${safe}%'
           OR o.description LIKE '%${safe}%'
           OR (JSON_SEARCH(o.tags, 'one', '%${safe}%') IS NOT NULL)
           OR u.username LIKE '%${safe}%'
           OR u.display_name LIKE '%${safe}%'
         )
       ORDER BY name_sim DESC, r.avg_rating DESC
       LIMIT ${limit} OFFSET ${offset}`
    )
  }
  ...
  async saveToBoard(boardId, overlayId, note = null) {
    ...
    return this.query(
      `INSERT IGNORE INTO board_items (board_id, overlay_id, note)
    ...
    return this.queryOne(
      `INSERT INTO reviews (overlay_id, user_id, purchase_id, rating, title, body)
       ...
       ON DUPLICATE KEY UPDATE
         rating = VALUES(rating), title = VALUES(title),
         body = VALUES(body)
